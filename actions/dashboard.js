...
// ⬇️ import dynamically: prevents bundling Pinecone/AI into client build
const getAI = async () => (await import("@/lib/aiInsights")).generateAIInsights;
...
export async function getIndustryInsights() {
  ...
  if (!user.industryInsight) {
    const generateAIInsights = await getAI();
    const insights = await generateAIInsights(user.industry);
